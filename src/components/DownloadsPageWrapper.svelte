  ...
  async function fetchBuilds() {
    try {
      ...
      const response = await fetch(
        `/api/v2/builds?experimental=true${projectParam}`
      );
      ...
      const filteredBuilds = builds.filter(
        (b) => b.channelVersion !== "unknown"
      );

      buildsByVersion = filteredBuilds.reduce<Record<string, Build[]>>(
        (grouped, build) => {
...
{#if loading}
  <section class="mt-12 sm:mt-16 min-h-[60vh] mb-20">
    <div
      class="border border-neutral-800 rounded-lg p-6 relative bg-neutral-900/30 backdrop-blur-sm overflow-hidden"
    >
      <div
        class="flex flex-col items-center justify-center py-24 text-center overflow-hidden"
      >
