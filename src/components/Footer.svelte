<script lang="ts">
  ...
  const LINKS = {
    projectDev: [
      {
        ...
        labelKey: "footer.links.githubRepo",
      },
      { href: jenkinsConfig.baseUrl, labelKey: "footer.links.jenkins" },
      { href: "/downloads", labelKey: "footer.links.downloads" },
      {
        href: "https://github.com/CraftCanvasMC/Website/blob/main/docs/API.md",
        labelKey: "footer.links.apiDocs",
      },
    ],
    getInvolved: [
      {
    ...
    aboutCanvas: [
      {
        href: `${siteConfig.links.github.repo}/blob/master/LICENSE`,
...
<footer
  class="relative mt-10 border-neutral-800/80 border-t lg:mt-14 bg-[var(--background)] clear-both"
>
  <div class="container mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
    <div class="grid gap-8 sm:grid-cols-2 sm:gap-10 lg:grid-cols-5">
      <section class="text-left lg:col-span-2">
        <a
          href="/"
          class="inline-flex items-center gap-2 rounded-xl hover:opacity-90"
        >
        ...
        <div class="mt-4 flex gap-4">
          {#each SOCIALS as { href, icon, labelKey } (labelKey)}
      ...
      <div class="grid gap-8 sm:grid-cols-2 lg:col-span-3 lg:grid-cols-3">
        {#each Object.entries(LINKS) as [sectionKey, links] (sectionKey)}
    ...
    <div class="mt-8 space-y-4 border-neutral-800/80 border-t pt-8">
      ...
      >
        ...
        <div class="flex items-center gap-1 text-neutral-400">
          {$t("footer.builtWith")}
          <Heart class="size-3" fill="currentColor" />
          {$t("footer.by")}
